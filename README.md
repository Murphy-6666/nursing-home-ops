...
自动汇总体征趋势、收入构成、排班分布、活动参与率，一屏掌握全院状况：

![运营仪表盘](./images/dashboard-preview.jpg)

### 护理记录表

语音录入后 AI 自动提取体征数据，异常值自动标红预警：

![护理记录表](./images/care-records-table.jpg)

### 家属每日推送

每天定时自动推送老人情况，家属不用再打电话问：

![家属推送](./images/family-notification.jpg)
...
> 本系统为辅助管理工具，仅用于日常运营记录和数据整理，不替代医疗诊断、护理判断或用药建议。实际用药须以医嘱为准，健康预警可能存在误报或漏报，使用方需自行核验数据准确性。详细声明见 [LICENSE](./LICENSE)。
